...
class RabbitMQProvider {
  ...
  constructor(params, logger) {
    ...
    self.options = tools.validate(params, {
      url: {
        optional: true,
        type: 'string',
  ...
  ack(ackid) {
    const self = this;
    return self._exec({ackid: ackid}, {ackid: 'string'}, (vp) => {
      self.channel.ack(self.messages[vp.ackid].msg);
    });
  }
  ...
  nack(ackid) {
    const self = this;
    return self._exec({ackid: ackid}, {ackid: 'string'}, (vp) => {
      self.channel.nack(self.messages[vp.ackid].msg);
    });
  }
   ...
   * @param {function} params.ack - ack mode
   * auto: as soon as the message is consumed
   * resolve: as soon as the callback is resolved
   * if none, you should ack manually by calling provider's ack method
   * @return {object} - promise
   */
  consume(params) {
    const self = this;
      ...
      return new Promise((resolve, reject) => {
        self.channel.assertQueue(vp.queue, {durable: true, autoDelete: false}, function (qErr, qData) {
          ...
          self.channel.consume(vp.queue, function (msg) {
            const json = self._json(msg);
            self.logger.debug('consume: received new message, ', json);
            const res = vp.cb(json);
            if (res instanceof Promise) {
              res.then(function () {
                self.logger.debug('resolved consume callback');
                if (vp.ack === 'resolve') {
          ...
          self.channel.assertQueue(vp.key, {durable: true, autoDelete: false}, function (qErr, qData) {
            ...
            self.channel.bindQueue(qData.queue, vp.key, '', {}, function (bErr) {
              ...
              self.channel.consume(qData.queue, function (msg) {
                const json = self._json(msg);
                self.logger.debug('subscribe: received new message, ', json);
                const res = vp.cb(json);
                if (res instanceof Promise) {
                  res.then(function () {
                    self.logger.debug('resolved subscribe callback');
                    if (vp.ack === 'resolve') {
                      self.channel.ack(msg);
                      self.logger.debug('acknowledged message: ', json);
                    }
