...
class IoBrick extends Brick {
  ...
  constructor(cementHelper, config) {
    ...
    if (this.properties.input && typeof this.properties.input === 'object') {
      ...
      if (this.properties.input.key) {
        this.input.key = this.properties.input.key;
      }
    }
    this.output = {
      queue: null,
    ...
    if (this.properties.output && typeof this.properties.output === 'object' && this.properties.output.queue) {
      ...
      if (this.properties.output.key) {
        this.output.key = this.properties.output.key;
      }
    }
    /*that.inputQueue = that.properties.inputQueue || null;
    that.inputKey = that.properties.inputKey || null;
    // that.inputMethod = that.properties.inputMethod || 'subscribe';
    that.outputMethod = that.properties.outputMethod || 'produce';
  ...
  start() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const keys = Array.isArray(that.input.key) ? that.input.key : [that.input.key];
      const cb = (json) => {
  ...
  process(context) {
    ...
    return co(function* processCoroutine() {
      ...
      // subscribe
        ...
        const res = yield that._subscribe(payload.key);
        context.emit('done', that.name, res);

      // consume
      } else if (type === 'message' && quality === 'consume') {
        const res = yield that._consume(payload.queue);
        context.emit('done', that.name, res);

      // publish
      } else if (type === 'message' && quality === 'publish') {
        const res = yield that._publish(payload.key);
        context.emit('done', that.name, res);
  ...
  _produce(data) {
    const that = this;
    const queue = data.payload.queue || that.outputQueue;
    const message = (data.payload.queue && data.payload.message) ? data.payload.message : data.payload;
    return new Promise((resolve, reject) => {
