...
class RQProvider extends EventEmitter {
  ...
  _reconnect() {
    ...
    if (self.reconnecting === true) {
      self.logger.debug('*** already reconnecting...');
      return;
    }
      ...
      amqp.connect(self.options.url, function(connErr, connection) {
        if (connErr) {
          ...
          connection.on('close', function() {
            self.logger.debug('*** connection close event');
            self.connected = false;
            self._reconnect();
          });
          connection.on('error', function(err) {
            self.logger.debug('*** connection error event: ', err);
          });
          ...
          connection.createConfirmChannel(function(chErr, channel) {
            if (chErr) {
              ...
              channel.on('close', function() {
                self.logger.debug('*** channel close event');
                self._reconnect();
              });
              channel.on('error', function(err) {
                self.logger.debug('*** channel error event: ', err);
              });
      ...
      try {
        channel.assertQueue(params.queue, {durable: true, autoDelete: false}, function(qErr, qData) {
          ...
          channel.sendToQueue(params.queue, self._jsonToBuffer(params.json), {persistent: true}, function(sErr) {
            ...
            self.logger.debug('Produced new message: ', params.json);
            resolve(qData);
          });
        });
      } catch (e) {
  ...
  get(channel, params) {
    ...
    return new Promise((resolve, reject) => {
      try {
        channel.prefetch(1);
        self.logger.debug('Getting a message in queue "%s"', params.queue);
      ...
      try {
        channel.assertQueue(params.queue, {durable: true, autoDelete: false}, function(qErr, qData) {
          ...
          channel.consume(params.queue, function(msg) {
            ...
            self.logger.debug(`consume: starting new consumer with id ${cData.consumerTag}, waiting for messages in queue ${params.queue}`);
      ...
      try {
        channel.assertExchange(params.key, 'fanout', {durable: true, autoDelete: false}, function(aErr, aData) {
          ...
          channel.publish(params.key, '', self._jsonToBuffer(params.json), {persistent: true}, function(pErr) {
            ...
            self.logger.debug('Published new message: ', params.json);
            resolve(aData);
          });
        });
      } catch (e) {
          ...
          channel.assertQueue(params.key, {durable: true, autoDelete: false}, function(qErr, qData) {
            ...
            channel.bindQueue(qData.queue, params.key, '', {}, function(bErr) {
              ...
              channel.consume(qData.queue, function(msg) {
                ...
                self.logger.debug(`subscribe: starting new consumer with id ${cData.consumerTag}, waiting for messages in queue ${params.key}`);
