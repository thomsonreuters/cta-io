...
class WKProvider {
  ...
  constructor(params) {
    ...
    self.options = validate(params, pattern);
    self.connection = {
      queue: null,
    };
    self.rejects = [];
  ...
  _exec(cb) {
    ...
    return new Promise((resolve, reject) => {
      const connection = new autobahn.Connection({
        url: self.options.wamp.url,
        realm: self.options.wamp.realm,
      });
  ...
  connect() {
    ...
    if (self.connection.queue === null) {
      self.connection.queue = kue.createQueue(self.options.kue);
      self.connection.queue.on( 'error', function(err) {
        console.error('cta-io -> Oops...');
        self.reject();
      });
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        connection.queue.process(params.queue, function(job, done) {
          console.log('cta-io -> Found task, processing...');
  ...
  publish(connection, params) {
    return this._exec(function(session) {
    ...
    return this._exec(function(session) {
      return new Promise((resolve, reject) => {
        session.subscribe(params.key, function(data) {
          ...
          console.log('cta-io -> Subscribed to ' + params.key);
          resolve('ok');
        }, function(err) {
          console.error('cta-io -> Failed to subscribe to ' + params.key, err);
          reject(err);
        });
      });
    });
