    ...
    return new Promise((resolve, reject) => {
      try {
        connection.queue.process(params.queue, function(job, done) {
          console.log('SQR => Found task, processing...');
    ...
    return this._exec(function(session) {
      return new Promise((resolve, reject) => {
        session.subscribe(params.key, function(data) {
          ...
          console.log('SQR => Subscribed to ' + params.key);
          resolve('ok');
        }, function(err) {
          console.error('SQR => Failed to subscribe to ' + params.key, err);
          reject(err);
        });
      });
    });
